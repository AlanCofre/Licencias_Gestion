...
export const crearCurso = async (req, res) => {
  try {
    const { codigo, nombre_curso, semestre, seccion, periodo, id_usuario } = req.body;
    ...
    if (profesor.rol !== ROL_PROFESOR && profesor.nombre_rol !== ROL_PROFESOR && profesor.id_rol !== 1) {
      return fail(res, 'El usuario indicado no tiene rol de profesor', 400);
    }

    // validar unicidad (codigo + seccion + periodo)
    const existe = await Curso.findOne({ where: { codigo, seccion, periodo } });
    if (existe) return fail(res, 'Ya existe un curso con ese código, sección y período', 409);

    const nuevo = await Curso.create({ codigo, nombre_curso, semestre, seccion, periodo, id_usuario });
    return res.status(201).json({ ok: true, mensaje: 'Curso creado correctamente', data: nuevo });
...
export const actualizarCurso = async (req, res) => {
  try {
    ...
    const existe = await Curso.findOne({ where: { codigo: codigoFinal, seccion: seccionFinal, periodo: periodoFinal } });
...
export const listarCursos = async (req, res) => {
  try {
    const cursos = await Curso.findAll({
      include: [
        { model: Usuario, as: 'profesor', attributes: ['id_usuario', 'nombre', 'correo', 'id_rol'] }
      ],
      order: [['codigo', 'ASC']]
    });
